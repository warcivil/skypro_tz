...
class Product(models.Model):
    ...
    def orders_last_month(self):
        today = timezone.now()
        last_month_start = today.replace(month=today.month-1, day=1, hour=0, second=0, minute=0)
        last_month_end = today.replace(day=1, hour=23, minute=59, second=59) - timedelta(days=1)
        return self.orders.filter(order_date__range=[last_month_start, last_month_end]).count()
